...
namespace Brasserie.Utilities.DataAccess
{
    public class DataAccessCsvFile : DataAccess , IDataAccess
    {
        ...
        {
            string[] fields = csvline.Split(';');
            if (!string.IsNullOrEmpty(fields[0]) && fields[0].Equals("STAFFMEMBER"))
            {
                StaffMember m = new StaffMember(id: int.Parse(fields[1]), lastName: fields[2], firstName: fields[3], gender: bool.Parse(fields[4]), email: fields[5], phone: fields[6], bankAccount: fields[7], address: fields[9], salary: double.Parse(fields[10]));
                return m;
            }
            else if (!string.IsNullOrEmpty(fields[0]) && fields[0].Equals("MANAGER"))
            {
                Manager m = new Manager(id: int.Parse(fields[1]), lastName: fields[2], firstName: fields[3], gender: bool.Parse(fields[4]), email: fields[5], phone: fields[6], bankAccount: fields[7], address: fields[9], salary: double.Parse (fields[10]), password : fields[11]);
                return m;
            }
            else
            {
        ...
        public override StaffMembersCollection GetAllStaffMembers()
        {
            ...
            if (IsValidAccessPath)
            {
                ...
                foreach (string s in listToRead)
                {
                    StaffMember m = GetStaffMember(s);
                    if (m != null)
                    {
                        staffMembers.AddStaffmember(m);
                    }
                }
                return staffMembers;
            }
